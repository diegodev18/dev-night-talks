...
const communities: Community[] = [
  {
    ...
    description: 'Comunidad en torno a la nube AWS: buenas prácticas, arquitectura y experiencias locales.',
  ...
  },
  {
    ...
    description: 'Espacio para quienes usan Cursor y herramientas de desarrollo con asistencia de IA.',
  ...
  },
  {
    ...
    description: 'Encuentros y conversación en torno a Claude y el ecosistema de IA conversacional.',
...
function CommunityCardLink({ href, children }: { href: string; children: ReactNode }) {
  if (href.startsWith('/')) {
    return (
  ...
  if (/^https?:\/\//i.test(href)) {
    return (
      <a href={href} target="_blank" rel="noopener noreferrer" className={communityCardLinkClass}>
        {children}
      </a>
    );
  }
  ...
  const card = (
    ...
    >
      <div className="relative aspect-4/3 w-full bg-card">
        ...
        {imageFailed ? (
          <div className="flex size-full items-center justify-center bg-muted" aria-hidden>
            <span className="font-heading text-2xl font-semibold tracking-tight text-muted-foreground">{initials}</span>
          </div>
        ) : null}
        {isLinkable ? (
      ...
      <CardHeader className="border-0 px-3 pb-3 pt-3 sm:px-4 sm:pb-4">
        ...
        <CardDescription className="text-pretty text-xs sm:text-sm">{description}</CardDescription>
      </CardHeader>
    </Card>
  );
    ...
    <Layout>
      <DirectionalTransition>
        ...
        <main className="flex flex-col gap-10">
          <section className="flex flex-col gap-6" aria-labelledby="groups-page-heading">
            <div className="flex max-w-3xl flex-col gap-4">
              <h1 id="groups-page-heading" className="font-heading text-2xl font-semibold tracking-tight text-foreground sm:text-3xl">
                Comunidades hermanas
              </h1>
              <p className="text-pretty text-sm text-muted-foreground sm:text-base">
                Estos grupos tomaron forma o se fortalecieron junto al ecosistema de{' '}
                <span className="text-foreground">Dev Night Talks</span> en Villahermosa: espacios distintos con su propio enfoque, la misma
                energía de compartir y aprender en comunidad.
              </p>
              <p className="text-pretty text-sm text-muted-foreground sm:text-base">
                Cada uno organiza sus propios encuentros y dinámicas; aquí solo los damos a conocer para que puedas sumarte al que más te
                interese.
              </p>
            </div>
